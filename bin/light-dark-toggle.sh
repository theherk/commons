...
if grep -q "light = true" ~/.config/git/config; then
  ...
  sed -i '' 's/\(local selected_scheme = \)".*"/\1"tokyonight_night"/' .config/wezterm/wezterm.lua
  ...
  sed -i '' 's/\(local selected_scheme = \)".*"/\1"tokyonight_day"/' .config/wezterm/wezterm.lua
  sed -i '' 's/\(lvim.colorscheme = \)".*"/\1"tokyonight-day"/' .config/lvim/lua/user/display.lua
  sed -i '' 's/\(vim.g.tokyonight_style = \)".*"/\1"day"/' .config/lvim/lua/user/display.lua
