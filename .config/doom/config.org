...
Use a POSIX shell internally to avoid issues with Fish/Nushell in child processes, but keep Fish for terminal emulators.
...
Toggle between Catppuccin Frappe (dark) and Latte (light).

#+begin_src elisp
(defun h4s/toggle-theme ()
  "Toggle between Catppuccin Frappe and Latte."
  (interactive)
  (setq catppuccin-flavor
        (if (eq catppuccin-flavor 'frappe) 'latte 'frappe))
  (catppuccin-reload))

(map! :leader
  :desc "toggle light/dark"
  "t d" #'h4s/toggle-theme)
#+end_src
...
#+begin_src elisp
(defun h4s/jjui ()
  "Open jjui in a vterm buffer."
  (interactive)
  (let ((buf (generate-new-buffer "*jjui*")))
    (with-current-buffer buf
      (vterm-mode)
      (vterm-send-string "jjui; exit\n"))
